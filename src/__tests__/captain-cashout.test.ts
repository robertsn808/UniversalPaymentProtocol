...
import { createPaymentProcessor } from '../../server/stripe-integration';

// Mock the payment processor
vi.mock('../../server/stripe-integration', () => ({
  createPaymentProcessor: vi.fn()
}));

// Mock SecureFileHandler
vi.mock('../../src/utils/file-security', () => ({
  SecureFileHandler: {
...
describe('Captain Cashout Payment System', () => {
  ...
  beforeEach(() => {
    ...
    const { SecureFileHandler } = require('../../src/utils/file-security');
