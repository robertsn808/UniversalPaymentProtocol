...
// Product catalog (in-memory for demo, replace with database)
interface Product {
  id: string;
  name: string;
  price: number;
...
// Active orders storage (in-memory for demo)
...
router.get('/pos/products', asyncHandler(async (req, res) => {
...
router.post('/pos/order', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res) => {
...
router.post('/pos/payment', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    if (paymentMethod === 'card') {
      ...
      paymentResult = await paymentProcessor.processDevicePayment({
        amount: order.total,
    ...
    if (paymentResult.success) {
      ...
      res.json({
        ...
        receipt: generateReceipt(order, paymentResult)
      });
    } else {
      order.status = 'failed';
      res.status(400).json({
...
router.get('/pos/order/:orderId', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res) => {
...
router.post('/pos/refund', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res) => {
...
// Helper function to generate receipt (simple text for demo)
function generateReceipt(order: POSOrder, paymentResult: any): string {
  let receipt = `Receipt: ${order.receiptNumber}\nDate: ${order.createdAt.toLocaleString()}\n\nItems:\n`;
