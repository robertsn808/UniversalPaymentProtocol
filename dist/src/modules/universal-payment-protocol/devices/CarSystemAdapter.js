...
export class CarSystemAdapter {
    constructor(deviceInfo) {
        this.deviceInfo = deviceInfo;
        this.deviceType = 'car_system';
        this.capabilities = {
    ...
    async showSuccessOnDisplay(response) {
    ...
    async showErrorOnDisplay(error) {
    ...
    async showPaymentScreen(options) {
        console.log('🚗 Showing automotive payment screen:', options);
        if (options.safety_mode) {
            // Voice-only interface while driving
            await this.announceVoice(`Payment request: $${options.amount} for ${options.service_type}. Say confirm to proceed.`);
        }
        else {
    ...
    async handleSteeringWheelInput() {
        return new Promise((resolve) => {
    ...
    async activateParkingSession(response) {
    ...
    async logTollTransaction(response) {
    ...
    async handleDriveThrough(orderData) {
    ...
    async handleChargingPayment(chargingData) {
    ...
    async handleLocationBasedPayment(location) {
